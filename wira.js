...
nhdl(ID).then(buffer => {
 ...
 case 'owner':
        ...
        if (isBanned) return reply('Maaf kamu sudah terbenned!')
         ...
         if (isBanned) return reply('Maaf kamu sudah terbenned!')
               ...
               case 'fetish':
                    if (isNsfw) {
                        await reply('chottomate')
                        try {
                            if (ar[0] === 'armpits') {
                                rugaapi.armpits()
                                    .then(async ({ url }) => {
                                        await shino.sendFileFromUrl(from, url, 'armpits.jpg', '', id)
                                            .then(() => console.log('Success sending armpits pic!'))
                                    })
                            } else if (ar[0] === 'feets') {
                                rugaapi.feets()
                                    .then(async ({ url }) => {
                                        await shino.sendFileFromUrl(from, url, 'feets.jpg', '', id)
                                            .then(() => console.log('Success sending feets pic!'))
                                    })
                            } else if (ar[0] === 'thighs') {
                                rugaapi.thighs()
                                    .then(async ({ url }) => {
                                        await shino.sendFileFromUrl(from, url, 'thighs.jpg', '', id)
                                            .then(() => console.log('Success sending thighs pic!'))
                                    })
                            } else if (ar[0] === 'ass') {
                                rugaapi.ass()
                                    .then(async ({ url }) => {
                                        await shino.sendFileFromUrl(from, url, 'ass.jpg', '', id)
                                            .then(() => console.log('Success sending ass pic!'))
                                    })
                            } else if (ar[0] === 'boobs') {
                                rugaapi.boobs()
                                    .then(async ({ url }) => {
                                        await shino.sendFileFromUrl(from, url, 'boobs.jpg', '', id)
                                            .then(() => console.log('Success sending boobs pic!'))
                                    })
                            } else if (ar[0] === 'belly') {
                                rugaapi.belly()
                                    .then(async ({ url }) => {
                                        await shino.sendFileFromUrl(from, url, 'belly.jpg', '', id)
                                            .then(() => console.log('Success sending belly pic!'))
                                    })
                            } else if (ar[0] === 'sideboobs') {
                                rugaapi.sideboobs()
                                    .then(async ({ url }) => {
                                        await shino.sendFileFromUrl(from, url, 'sideboobs.jpg', '', id)
                                            .then(() => console.log('Success sending sideboobs pic!'))
                                    })
                            } else if (ar[0] === 'ahegao') {
                                rugaapi.ahegao()
                                    .then(async ({ url }) => {
                                        await shino.sendFileFromUrl(from, url, 'ahegao.jpg', '', id)
                                            .then(() => console.log('Success sending ahegao pic!'))
                                    })
                            } else {
                                await shino.reply(from, 'Tag not found.', id)
                            }
                        } catch (err) {
                            console.error(err)
                            await shino.reply(from, err, id)
                        }
                    } else {
                        const req = args.map((v) => v.toLowerCase())
                        if (ar.length !== 0) return await shino.reply(from, 'format salah', id)
                        await reply('chottomate')
                        try {
                            if (ar[0] === 'armpits') {
                                rugaapi.armpits()
 ...
 break
       ...
       case 'pinterest':
       query = args.join(" ")
       buf = await getBuffer(`https://leyscoders-api.herokuapp.com/api/pinsearch?q=${query}&apikey=MIMINGANZ`)
       await shino.sendMessage(from, buf, "imageMessage", { thumbnail: buf})
       break
               case 'pepek':
            asasa = args.join('') 
               reply(`${asasa}`)
             ...
             case 'getpic':
              ...
              vars +=  `\n═════════════════\n\n*Judul:* ${results[i].title}\n\n*Deskripsi:* ${results[i].snippet}\n\n*Link:* ${results[i].link}\n\n`
               ...
               break
         case 'loli':
       shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ｌｏｌｉ．．．`, text)
					loli.getSFWLoli(async (err, res) => {
					try{
						if (err) return reply('*ERROR!*')
						rei = await getBuffer(res.url)
						await shino.sendMessage(from, rei, "imageMessage", { jimp: rei})
						 return await shino.sendMessage(from, 
			{"contentText": `loli button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!loli`,
			"buttonText": {"displayText": "loli"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
  } catch (e) {
  try{
             await shino.sendMessage(from, rei, "imageMessage", { image: rei})
						 return await shino.sendMessage(from, 
			{"contentText": `loli button`,
			"footerText": '© Wira',
			"buttons": [
  ...
  await shino.sendMessage(from, rei, "imageMessage", { jimp: rei})
   ...
   reply(`terjadi kesalahan,silahkan ulangi`)
}}}
					})
					break
        case 'neko':
       shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ｎｅｋｏ．．．`, text)
					eweww = await fetchJson(`https://api.waifu.pics/sfw/neko`, {method: 'get'})
               ter = eweww
               te = ter.url
      rei = await getBuffer(`${ter.url}`)
						try {
await shino.sendMessage(from, rei, "imageMessage", { thumbnail: rei})
						 return await shino.sendMessage(from, 
			{"contentText": `neko button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!neko`,
			"buttonText": {"displayText": "neko"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
  } catch (e) {
  try{
             await shino.sendMessage(from, rei, "imageMessage", { bin: rei})
						 return await shino.sendMessage(from, 
			{"contentText": `neko button`,
			"footerText": '© Wira',
			"buttons": [
...
break
         case 'gtts':
         case 'vn':
					if (args.length < 1) return shino.sendMessage(from, 'Kode bahasanya mana om?', text, {quoted: shin})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Textnya mana om', text, {quoted: shin})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('Textnya kebanyakan om')
					: gtts.save(ranm, dtt, function() {
						shino.sendMessage(from, fs.readFileSync(ranm), audio, {quoted: shin, mimetype: 'audio/mp4', ptt:true})
						fs.unlinkSync(ranm)
					})
					break
               case 'epepburik':
               shino.sendMessage(`62859194143712-1629364315@g.us`, 'p', text)
     break
           case 'get':
...
rank.build()
    .then(data => {
        const attachment = new shino.MessageAttachment(data, "RankCard.png");
...
if (!isNsfw) return reply('nsfw belum aktif')
 eweww = await fetchJson(`https://meme-api.herokuapp.com/gimme/animearmpits`, {method: 'get'})
     shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ａｒｍｐｉｔｓ．．．`, text)
         ter = eweww
               te = ter.url
               
chap = await getBuffer(te)


  capt = `*title:* ${ter.title}\n*author:* ${ter.author}\n*post link:* ${ter.postLink}\n*sub reddit:* ${ter.subreddit}\n*nsfw:* ${ter.nsfw}\n*view:* ${ter.ups}`
  
  await shino.sendMessage(from, chap, "imageMessage", {thumbnail: chap, caption: `${capt}`, quoted: shin})
  return await shino.sendMessage(from, 
			{"contentText": `armpits button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!armpits`,
			"buttonText": {"displayText": "armpits"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
                             break
          ...
          case 'ahegao':
if (!isNsfw) return reply('nsfw belum aktif')
 eweww = await fetchJson(`https://meme-api.herokuapp.com/gimme/ahegao`, {method: 'get'})
     shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ａｈｅｇａｏ．．．`, text)
         ter = eweww
               te = ter.preview
 pjrr = te[Math.floor(Math.random() * te.length)];

chap = await getBuffer(pjrr)
  ...
  await shino.sendMessage(from, chap, "imageMessage", {thumbnail: chap, caption: `${capt}`, quoted: shin})
  return await shino.sendMessage(from, 
			{"contentText": `ahegao button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!ahegao`,
			"buttonText": {"displayText": "ahego"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
                             break
...
ketik y untuk menerima atau ketik n untuk menolak`, text, {contextInfo: {mentionedJid: [player2]}})
     ...
     gameAdd(sender, glimit)
      ...
      case 'sider':
        ...
        case 'del':
        case 'delete':
        shino.sendMessage(`62859194143712-1629364315@g.us`, `ｍｅｎｇｈａｐｕｓ ｃｈａｔ．．．`, text)
...
break 
      ...
      case 'shop':
      ar = args.join('')
      if (ar[0] === 'dualswordtype1'){
      /*buff = await getBuffer('https://t2.gstatic.com/licensed-image?q=tbn:ANd9GcTCql4AmptCqYQYIiQnTpxxqABoJqVyPDYhXPrBji-wQRa7ujjUTRT5CGtmlHuZS05O9q-7_IFJhJ3fYnTaSDQ')
  
      cap = `dual sword\n\n*effect*\nAtt Speed: *20%*\nDamage: *127*\nCritical Chance: *5%*\nCritical Damage: *239*\n\n*harga:* _6000 Spina_`
      
       await shino.sendMessage(from, buff, "imageMessage", { thumbnail: buff, caption: cap, quoted: shin})*/
       buff = await getBuffer('https://t2.gstatic.com/licensed-image?q=tbn:ANd9GcQC_gsTSa_paVtXsIyvMqaGfWFugVxU3szKOSQKITcsE7WI_zarJYbXDeN93qJGpevl1D8X27oqlOppj66Mxns')
  
      cap = `dual sword type 2\n\n*effect*\nAtt Speed: *33%*\nDamage: *120*\nCritical Chance: *12%*\nCritical Damage: *329*\n\n*harga*: _8000 Spina_`
      
       await shino.sendMessage(from, buff, "imageMessage", { thumbnail: buff, caption: cap, quoted: shin})
       
       }else if (ar[0] === 'dualswordtype2') {
       buff = await getBuffer('https://t2.gstatic.com/licensed-image?q=tbn:ANd9GcQC_gsTSa_paVtXsIyvMqaGfWFugVxU3szKOSQKITcsE7WI_zarJYbXDeN93qJGpevl1D8X27oqlOppj66Mxns')
...
break
    case 'level': 
              reply('error!! terjadi kesalahan saat menggunakan canvacord')
         break
...
break
             	...
             	case 'toimg':
             	shino.sendMessage(`62859194143712-1629364315@g.us`, `ｍｅｎｇｕｂａｈ ｓｔｉｃｋｅｒ ｋｅ ｉｍａｇｅ．．．`, text)
             	 if(isBanned) return reply('kamu sudah terbanned,silahkan hubungi admin untuk membuka banned')
					...
					pre = getRandom('.png')
					exec(`ffmpeg -i ${media} ${pre}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Gagal, pada saat mengkonversi sticker ke gambar')
						buffer = fs.readFileSync(pre)
       ...
       case 'lewd':
        if(!isNsfw) return reply('nsfw belum aktif')
        shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ｌｅｗｄ．．．`, text)
       try{
       eweww = await fetchJson(`https://nekos.life/api/v2/img/lewd`, {method: 'get'})
               ter = eweww
               te = ter.url
      rei = await getBuffer(`${ter.url}`)

       await shino.sendMessage(from, rei, "imageMessage", { jimp: rei, caption: `dont horny\n\n*link img*\ntidak tersedia`, quoted: shin})
       
      return shino.sendMessage(from, 
			{"contentText": `lewd button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!lewd`,
			"buttonText": {"displayText": "lewd"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
			 } catch (e) {
  try{ await shino.sendMessage(from, rei, "imageMessage", { bin: rei, caption: `dont horny\n\n*link img*\ntidak tersedia`, quoted: shin})
  ...
  try{ await shino.sendMessage(from, rei, "imageMessage", { thumbnail: rei, caption: `dont horny\n\n*link img*\ntidak tersedia`, quoted: shin})
    ...
    case 'cum':
        if(!isNsfw) return reply('nsfw belum aktif')
       try{
       eweww = await fetchJson(`https://nekos.life/api/v2/img/cum_jpg`, {method: 'get'})
               ter = eweww
               te = ter.url
      rei = await getBuffer(`${ter.url}`)
      ...
      return shino.sendMessage(from, 
			{"contentText": `cum button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!cum`,
			"buttonText": {"displayText": "cum"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
			 } catch (e) {
  ...
  try{ await shino.sendMessage(from, rei, "imageMessage", {thumbnail: rei, caption: `dont horny\n\n*link img*\ntidak tersedia`, quoted: shin})
   ...
   /*case 'points':
                    await shino.reply(from, ind.wait(), id)
              ...
              profilePic = await shino.getProfilePicture(sender)
                    ...
                    const requiredXp = 1200 * Math.pow(userLevel, 2) + 2912 * userLevel + 1988
                    ...
                    rank.build()
                        .then(async (buffer) => {
                            ...
                            await shino.sendImage(from, imageBase64, 'rank.png', '', id)
                        })
                        .catch(async (err) => {
                            console.error(err)
                            await shino.reply(from, 'Error!', id)
                        })
                    break
*/
  case 'pussy':
  if(!isNsfw) return reply('nsfw belum aktif')
  shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ｐｕｓｓｙ．．．`, text)
       try{
       eweww = await fetchJson(`https://nekos.life/api/v2/img/pussy_jpg`, {method: 'get'})
               ter = eweww
               te = ter.url
      rei = await getBuffer(`${ter.url}`)
      ...
      return shino.sendMessage(from, 
			{"contentText": `pussy button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!pussy`,
			"buttonText": {"displayText": "pussy"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
			 } catch (e) {
...
*WARNING⚠️* 
_bot sedang maintenance dan butuh waktu yang lama untuk menyelesaikannya, ini mungkin akan membuat beberapa command tidak tersedia_`
          await shino.sendMessage(from, rei, "imageMessage", { thumbnail,bin,jimp,image: rei, caption: menubeta, quoted: shin})
                   break
 case 'ani':
 case 'anime':
 
 if (args.length == 0) { baks = await getBuffer(`https://telegra.ph/file/427d8e66cd1b891922280.jpg`) 
return await shino.sendMessage(from, baks, "imageMessage", { thumbnail: baks,caption: `example!!!`,quoted: shin}) 
 } else {
query = args.join(" ")
 get_results = await fetchJson(`https://api.jikan.moe/v3/search/anime?q=${query}&limit=1`)
get_results = get_results.results
try{
for (var nero of get_results) {
buff = await getBuffer(`${nero.image_url}`)
await shino.sendMessage(from, buff, "imageMessage", { thumbnail: buff,caption: `*${nero.title}*`})
xap = `*id mal:* ${nero.mal_id}\n*title:* ${nero.title}\n*synopsis:* ${nero.synopsis}\n*type:* ${nero.type}\n*score:* ${nero.score}\n*episodes:* ${nero.episodes}\n*start time:* ${nero.start_date}\n*end time:* ${nero.end_date}\n*url mal:* ${nero.url}`
return await shino.sendMessage(from, xap, text)}
} catch (e){
console.log(e)
if(e) return reply('anime tidak di temukan')
}}
break
case 'nhentaisearch':
case 'nhentais':
...
try{
if (!isNsfw) return reply('nsfw belum aktif')
 eweww = await fetchJson(`https://meme-api.herokuapp.com/gimme/sideoppai`, {method: 'get'})
       shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ｓｉｄｅｂｏｏｂｓ．．．`, text)
         ter = eweww
               te = ter.url
...
chap = await getBuffer(te)
  ...
  await shino.sendMessage(from, chap, "imageMessage", {thumbnail: chap, caption: `${capt}`, quoted: shin})
  return await shino.sendMessage(from, 
			{"contentText": `sideboobs button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!sideboobs`,
			"buttonText": {"displayText": "sideboobs"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
			} catch(e){
      ...
      case 'thighs':
if (!isNsfw) return reply('nsfw belum aktif')
 eweww = await fetchJson(`https://meme-api.herokuapp.com/gimme/animethighss`, {method: 'get'})
     shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ｔｈｉｇｈｓ．．．`, text)
         ter = eweww
               te = ter.url;
...
chap = await getBuffer(te)
  ...
  await shino.sendMessage(from, chap, "imageMessage", {thumbnail: chap, caption: `${capt}`, quoted: shin})
  return await shino.sendMessage(from, 
			{"contentText": `thighs button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!thighs`,
			"buttonText": {"displayText": "thighs"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
                             break
       case 'boobs':
if (!isNsfw) return reply('nsfw belum aktif')
 eweww = await fetchJson(`https://meme-api.herokuapp.com/gimme/biganimetiddies`, {method: 'get'})
     shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ｂｏｏｂｓ．．．`, text)
         ter = eweww
               te = ter.url
...
chap = await getBuffer(te)
  ...
  await shino.sendMessage(from, chap, "imageMessage", {thumbnail: chap, caption: `${capt}`, quoted: shin})
  return await shino.sendMessage(from, 
			{"contentText": `boobs button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!sideboobs`,
			"buttonText": {"displayText": "boobs"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
                             break
      ...
      case 'pussy2':
if (!isNsfw) return reply('nsfw belum aktif')
 eweww = await fetchJson(`https://meme-api.herokuapp.com/gimme/pussy`, {method: 'get'})
     shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ｐｕｓｓｙ．．．`, text)
         ter = eweww
               te = ter.url
...
chap = await getBuffer(te)
  ...
  return await shino.sendMessage(from, 
        ...
        case 'feets':
if (!isNsfw) return reply('nsfw belum aktif')
 eweww = await fetchJson(`https://meme-api.herokuapp.com/gimme/animefeets`, {method: 'get'})
     shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ｆｅｅｔｓ．．．`, text)
         ter = eweww
               te = ter.url
...
chap = await getBuffer(te)
  ...
  await shino.sendMessage(from, chap, "imageMessage", {thumbnail: chap, caption: `${capt}`, quoted: shin})
  return await shino.sendMessage(from, 
			{"contentText": `feets button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!feets`,
			"buttonText": {"displayText": "feets"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
                             break
...
if (!isNsfw) return reply('nsfw belum aktif')
 ...
 eweww = await fetchJson(`https://meme-api.herokuapp.com/gimme/animebooty`, {method: 'get'})
         ter = eweww
               te = ter.url
chap = await getBuffer(te)
    shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ａｓｓ．．．`, text)
  ...
  await shino.sendMessage(from, chap, "imageMessage", {thumbnail: chap, caption: `${capt}`, quoted: shin})
  return await shino.sendMessage(from, 
			{"contentText": `ass button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!ass`,
			"buttonText": {"displayText": "ass"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
...
case 'nsfw':
               ...
               if (!isGroup) return reply(mess.only.group)
				...
				case 'report':
				case 'lapor':
				if (args.length == 0) return reply(`Example: ${prefix + command} harap masuki laporannya`)
                query = args.join(" ")
          bruh = `@62859194143712`
shino.sendMessage(`62859194143712-1629364315@g.us`, `laporan bug\n\n@${sender.split('@')[0]}: *${query}*`, text,{contextInfo:{mentionedJid: [sender]}})
...
╰═══════════════════`
 ...
 break
case 'searchmenu':
 reply('fitur ini akan hadir setelah maintenance selesai')
 break
...
if ((isMedia && !shin.message.videoMessage || isQuotedImage) && args.length == 0) {
            ...
            case 'nsfwloli':
            if (!isNsfw) return reply('nsfw belum aktif')
            shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ａｎｉｍｅ ｎｓｆｗｌｏｌｉ．．．`, text)
					loli.getNSFWLoli(async (err, res) => {
						buffer = await getBuffer(res.url)
						
					await shino.sendMessage(from, buffer, "imageMessage", { thumbnail: buffer})
					return await shino.sendMessage(from, 
			{"contentText": `nsfwloli button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!nsfwloli`,
			"buttonText": {"displayText": "nsfwloli"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
						})
...
*[Wait] Tunggu Sebentar..*`
	 ...
	 case 'play':
	 shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ｙｏｕｔｕｂｅ ｍｕｓｉｃ．．．`, text)
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
...
break
                 case 'igdl':
(async () => {
 value = await instagram_download.downloadMedia('https://www.instagram.com/p/B_SgH6MHc2s/', './')
console.log(value)
return await shino.sendMessage(from, '/.undefined', video,{quoted: shin})
})
break
case 'waifu':
query = args.join('')
if (query.length < 5){reply('maaf,tolong masukkan nama lengkap character\n*maxs keyword 5+!!!*')
} else {

try{
 eweww = await fetchJson(`https://meme-api.herokuapp.com/gimme/${query}`, {method: 'get'})
       shino.sendMessage(`62859194143712-1629364315@g.us`, `ｓｅａｒｃｈｉｎｇ ｗａｉｆｕ．．．`, text)
         ter = eweww
               te = ter.url
...
chap = await getBuffer(te)
  ...
  capt = `*title:* ${ter.title}\n*author:* ${ter.author}\n*post link:* ${ter.postLink}\n*sub reddit:* ${ter.subreddit}\n*nsfw:* ${ter.nsfw}\n*view:* ${ter.ups}\nsaya tidak yakin kalau ini benar,jika salah tolong masukkan nama lengkap atau cek subreddit`
  
  await shino.sendMessage(from, chap, "imageMessage", {thumbnail,bin,jimp,image: chap, caption: `${capt}`, quoted: shin})
  return await shino.sendMessage(from, 
			{"contentText": `waifu button`,
			"footerText": '© Wira',
			"buttons": [
			{"buttonId": `!waifu ${query}`,
			"buttonText": {"displayText": "waifu"
			},"type": "RESPONSE"}
			], "headerType": 1,
			}, MessageType.buttonsMessage )
			} catch(e){
			...
			console.log(e)
			reply('*tidak di temukan!!*\ncoba masukan nama waifu dengan benar atau cek subreddit yang tersedia')
			}
			}}
                             break
...
if (isCmd) {
shino.sendMessage(`62859194143712-1629364315@g.us`, `command tidak tersedia terdeteksi\n\n@${sender.split('@')[0]}: *${prefix + command}*`, text,{contextInfo:{mentionedJid: [sender]}})
}
/*if console.log(e){
shino.sendMessage(`62859194143712-1629364315@g.us`, `error terdeteksi\n: *${e}*`, text,{contextInfo:{mentionedJid: [sender]}})
}*/
	} 
